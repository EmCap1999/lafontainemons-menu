...
export async function selectAll(db: DrizzleDatabase) {
	return db.select().from(schema.subsection).orderBy(asc(schema.subsection.name));
}
...
export async function selectWithSection(db: DrizzleDatabase, subsectionId: number) {
...
export async function selectBySectionWithItems(db: DrizzleDatabase, sectionId: number) {
	return db.query.subsection.findMany({
		where: eq(schema.subsection.sectionId, sectionId),
		with: {
			items: {
...
export async function insert(db: DrizzleDatabase, subsection: schema.SubsectionInsert) {
