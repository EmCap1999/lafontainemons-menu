...
export async function selectAll(db: DrizzleDatabase) {
	return db.select().from(schema.item).orderBy(asc(schema.item.price), asc(schema.item.name));
}
...
export async function selectBySubsection(db: DrizzleDatabase, subsectionId: number) {
	return db
		.select()
		.from(schema.item)
...
export async function selectBySectionWithoutSubsection(db: DrizzleDatabase, sectionId: number) {
	return db
		.select()
		.from(schema.item)
...
export async function selectAvailableBySection(db: DrizzleDatabase, sectionId: number) {
	return db
		.select()
		.from(schema.item)
...
export async function remove(db: DrizzleDatabase, itemId: number) {
	return db.delete(schema.item).where(eq(schema.item.itemId, itemId)).returning();
}

export async function countBySection(db: DrizzleDatabase, sectionId: number): Promise<number> {
	const result = await db
