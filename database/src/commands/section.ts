...
export async function selectAll(db: DrizzleDatabase) {
	return db.select().from(schema.section).orderBy(asc(schema.section.displayOrder));
}
...
export async function insert(db: DrizzleDatabase, section: schema.SectionInsert) {
...
export async function remove(db: DrizzleDatabase, sectionId: number) {
	return db.delete(schema.section).where(eq(schema.section.sectionId, sectionId)).returning();
}

export async function count(db: DrizzleDatabase): Promise<number> {
	const result = await db.select({ count: schema.section.sectionId }).from(schema.section);
